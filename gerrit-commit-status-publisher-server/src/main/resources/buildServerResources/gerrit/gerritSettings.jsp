...
<jsp:useBean id="keys" class="com.callfire.teamcity.plugin.gerrit.Constants"/>
  ...
  <tr>
    <th><label for="${keys.gerritNoVote}">Started Build Vote:<l:star/></label></th>
    <td>
      <props:textProperty name="${keys.gerritNoVote}" className="mediumField"/>
      <span class="error" id="error_${keys.gerritNoVote}"></span>
      <c:if test="${testConnectionSupported}">
        <script>
            $j(document).ready(function() {
                PublisherFeature.showTestConnection();
